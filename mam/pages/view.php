...
<h1 style="display:none"><?php switch ($resource["archive"])
	{
	case -2:
...
<h2 style="display:none" id="resourcetools"><?php echo $lang["resourcetools"]?></h2>
...
elseif ($is_template)
	...
	{ 
	?>
<table cellpadding="0" cellspacing="0" <?php if (checkperm("apr")) { ?> style="display:none" <?php } else {?> style="width:100%" <?php } ?> >
<tr>
<?php
...
elseif (strlen($resource["file_extension"])>0 && !($access==1 && $restricted_full_download==false))
	...
	if (file_exists($path))
		...
		{
		?>
			<tr style="width:100%;background-color:#ce3f38" class="DownloadDBlend">
			<td style="width:100%"><h2>Không tìm thấy file tư liệu gốc</h2></td>
			</tr>
		<?php
		}
	} 
...
<?php if (!hook("replaceresourceactions")) {?>
	...
	<li class="icon-angle-circled-right"><?php echo add_to_collection_link($ref,$search)?><?php echo $lang["action-addtocollection"]?></a></li>
	<?php if ($search=="!collection" . $usercollection) { ?><li class="icon-angle-circled-right"><?php echo remove_from_collection_link($ref,$search)?><?php echo $lang["action-removefromcollection"]?></a></li><?php } ?>
	<?php } ?>
	
	
	<?php if ($allow_share && ($access==0 || ($access==1 && $restricted_share))) { ?>
		<li class="icon-angle-circled-right"><a href="<?php echo $baseurl_short?>pages/resource_email.php?ref=<?php echo urlencode($ref)?>&search=<?php echo urlencode($search)?>&offset=<?php echo urlencode($offset)?>&order_by=<?php echo urlencode($order_by)?>&sort=<?php echo urlencode($sort)?>&archive=<?php echo urlencode($archive)?>"  onClick="return CentralSpaceLoad(this,true);"><?php echo $lang["emailresource"]?></a></li>	
	<?php if (!$hide_resource_share_link) { ?>
		<li class="icon-angle-circled-right"><a href="<?php echo $baseurl_short?>pages/resource_share.php?ref=<?php echo urlencode($ref) ?>" onClick="return CentralSpaceLoad(this,true);" ><?php echo $lang["share"]?></a></li>
   <?php } } ?>
	<?php if (($edit_access) && !checkperm("apr")) { ?>
		<li class="icon-angle-circled-right"><a href="edit.php?ref=<?php echo urlencode($ref)?>&search=<?php echo urlencode($search)?>&offset=<?php echo urlencode($offset)?>&order_by=<?php echo urlencode($order_by)?>&sort=<?php echo urlencode($sort)?>&archive=<?php echo urlencode($archive)?>"><?php echo $lang["action-edit"]?></a>
		</li>
		
		<?php if ($metadata_download)	{ ?>
		<li class="icon-angle-circled-right"><a href="<?php echo $baseurl_short?>pages/metadata_download.php?ref=<?php echo urlencode($ref)?>" onClick="return CentralSpaceLoad(this,true);" ><?php echo $lang["downloadmetadata"]?></a></li>
	<?php } ?>
	<?php if ((!checkperm("D") || hook('check_single_delete')) && !(isset($allow_resource_deletion) && !$allow_resource_deletion)){?><li class="icon-angle-circled-right"><a href="<?php echo $baseurl_short?>pages/delete.php?ref=<?php echo urlencode($ref)?>&search=<?php echo urlencode($search)?>&offset=<?php echo urlencode($offset)?>&order_by=<?php echo urlencode($order_by)?>&sort=<?php echo urlencode($sort)?>&archive=<?php echo urlencode($archive)?>" onClick="return CentralSpaceLoad(this,true);"><?php if ($resource["archive"]==3){echo $lang["action-delete_permanently"];} else {echo $lang["action-delete"];}?></a><?php } ?></li>
	
	<?php } ?>
	<?php if (checkperm("e" . $resource["archive"]) && !checkperm("apr")) { ?><li class="icon-angle-circled-right"><a href="<?php echo $baseurl_short?>pages/log.php?ref=<?php echo urlencode($ref)?>&search=<?php echo urlencode($search)?>&offset=<?php echo urlencode($offset)?>&order_by=<?php echo urlencode($order_by)?>&sort=<?php echo urlencode($sort)?>&archive=<?php echo urlencode($archive)?>" onClick="return CentralSpaceLoad(this,true);"><?php echo $lang["log"]?></a></li><?php } ?>
	<?php if (checkperm("R") && $display_request_log_link) { ?><li class="icon-angle-circled-right"><a href="<?php echo $baseurl_short?>pages/request_log.php?ref=<?php echo urlencode($ref)?>&search=<?php echo urlencode($search)?>&offset=<?php echo urlencode($offset)?>&order_by=<?php echo urlencode($order_by)?>&sort=<?php echo urlencode($sort)?>&archive=<?php echo urlencode($archive)?>" onClick="return CentralSpaceLoad(this,true);"><?php echo $lang["requestlog"]?></a></li><?php } ?><?php
...
<?php if (checkperm("apr")) { ?>
<form id="apr1" method="post" action="<?php echo $baseurl;?>/pages/update_apr.php?ref=<?php echo urlencode($ref)?>&userref=<?php echo urlencode($userref)?>">
<div class="approve" style="margin-top:10px">
<?php if ($resource["resource_type"]==6) {?>
	<div class="clearerleft"> </div>
	<label for="pheduyet">Thành phẩm này chưa được duyệt, hãy điền nội dung phê duyệt</label>
	<div class="clearerleft"> </div>
	<textarea name="pheduyet" style="width:100%;margin-top:10px;height:90px"></textarea>
	<div class="clearerleft"> </div>
	<div class="btnapr" style="text-align:center">
		<input type="submit" class="duyetbutton" name="chapnhan" value="Duyệt" style="border-radius:3px;font-size:12px;line-height:1;padding:5px 10px;background:#007bb5;cursor:pointer;text-align:center;border:1px solid #007bb5;color:#fff;width:80px;margin-top:15px;margin-right:15px;margin-bottom:10px">
		<input type="submit" class="duyetbutton"name="khongchapnhan" value="Không Duyệt" style="border-radius:3px;font-size:12px;line-height:1;padding:5px 10px;background:#ce3f38;cursor:pointer;text-align:center;border:1px solid #ce3f38;color:#fff;width:auto;margin-top:15px">
	</div>
<?php } else if ($resource["resource_type"]==7) {?>
	<div class="clearerleft"> </div>
	<label>Thành phẩm này đã được duyệt, nếu muốn thay đổi hãy click vào nút bên dưới</label>
	<div class="btnapr" style="text-align:center">
	<input type="submit" class="duyetbutton" name="huyduyet" value="Hủy lệnh Duyệt" style="border-radius:3px;font-size:12px;line-height:1;padding:5px 10px;background:#ce3f38;cursor:pointer;text-align:center;border:1px solid #ce3f38;color:#fff;width:auto;margin-top:15px;margin-bottom:10px">
	</div>
<?php } else if ($resource["resource_type"]==8) {?>
	<div class="clearerleft"> </div>
	<label>Thành phẩm này không được duyệt, nếu muốn thay đổi hãy click vào nút bên dưới</label>
	<div class="btnapr" style="text-align:center">
	<input type="submit" class="duyetbutton" name="huykhongduyet" value="Hủy lệnh Không Duyệt" style="border-radius:3px;font-size:12px;line-height:1;padding:5px 10px;background:#ce3f38;cursor:pointer;text-align:center;border:1px solid #ce3f38;color:#fff;width:auto;margin-top:15px;margin-bottom:10px">
	</div>
<?php } ?>
</div>
</form>
...
<div class="backtoresults" style="text-align:right">
...
<div id="viewright" style="height: 500px;overflow: auto;">
