<?php	
	...
	class ViewFactoryLoans extends Command {
		function doExecute( \MVC\Controller\Request $request ) {
			...
			$mFLTracking = new \MVC\Mapper\FactoryLoanTracking();
			$FLs = $mFLTracking->trackByFactory(array($IdFactory,$Today,$Today));
			$request->setObject('FLs', $FLs);
						
			$fToday = date("m/Y", strtotime($Today));
			$request->setProperty('Title', 'BẢNG THEO DÕI ỨNG TIỀN '.$Factory->getName().' THÁNG '.$fToday);
