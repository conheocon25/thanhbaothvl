...
class Schedule extends Mapper implements \MVC\Domain\ScheduleFinder{

    function __construct() {
        parent::__construct();			
		$tblReport = "ktth_schedule";
		...
		$updateStmt = sprintf("update %s set idposition=?, name=?, date_start=?, date_end=?, description=?, note=? where id=?", $tblReport);
		$insertStmt = sprintf("insert into %s ( idposition, name, date_start, date_end, description, note) values(?, ?, ?, ?, ?, ?)", $tblReport);
		...
		$findByPageStmt = sprintf("SELECT * FROM  %s where idposition=:idposition LIMIT :start,:max", $tblReport);
	...
	function findByPage( $values ){
		$this->findByPageStmt->bindValue(':idposition', (int)($values[0]), \PDO::PARAM_INT);
